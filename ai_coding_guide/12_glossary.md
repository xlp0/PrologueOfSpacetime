...
The 2026-onward framing that takes context engineering further: designing the *whole runtime* around the LLM — memory, guardrails, orchestration, context pipelines, multi-agent coordination. The discipline of building reliable agents in production. (Chapter 2, Chapter 3.)
...
Anything the harness reaches out to: the model provider's API, the local filesystem, MCP servers, external services. (Chapter 3.)
