...
An **agent skill** is a packaged bundle of know-how that teaches an agent how to do a particular kind of task well — without you having to re-explain it every time. If MCP (Chapter 7) gives an agent new *tools*, skills give an agent new *procedures*: when to use which tool, in what order, against what inputs, and what good looks like at the end.
