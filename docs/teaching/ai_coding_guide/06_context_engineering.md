...
A good harness automates a lot of this. But you, the human, influence it enormously — by what you put in your rules files, by how you scope your request, by whether you point the agent at the right files, by whether you let it flail or steer it. That influence is [[static-vs-dynamic-context|context engineering]].
...
You can use the best model in the world, in the best harness, with the best [[mcp-anthropic-standard|MCP]] servers and the best skills, and if the context window contains the wrong stuff, the agent will produce wrong output. Conversely, a mid-tier model with a beautifully curated context will often outperform a frontier model with a polluted one.
